...
class AttModel(CaptionModel):
    def __init__(self, args, tokenizer):
        ...
        if args.visual_extractor == "densenet121":  # d_cnn_features If ResNet-50/101: 2048. If DenseNet121: 1024. If ResNet18: 512
        ...
        # print(self.att_embed)  # for cnn visual feature embedding and input to Transformer

    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks)
        if self.args.mode == "CNN-TD":
            memory = att_feats  # embedding the cnn features
        elif self.args.mode == "CNN-Transformer":
            memory = self.model.encode(att_feats, att_masks)  # futher encoding with the Transformer encoder
    ...
    def core(self, it, memory, state, mask):  # xt,  memory[2, 98, 512], [], [2, 1, 98]
        ...
        out = self.model.decode(ys, memory, mask, subsequent_mask(ys.size(1)).to(memory.device))  # auto-gressive decoding
